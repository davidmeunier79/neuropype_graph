...
def create_pipeline_nii_to_conmat_simple(main_path, ROI_mask_file,pipeline_name = "nii_to_conmat",conf_interval_prob = 0.05,background_val=-1.0):
    ...
    inputnode = pe.Node(niu.IdentityInterface(fields=['nii_4D_file','rp_file']),name='inputnode')
    
    #### Nodes version: use min_BOLD_intensity and return coords where signal is strong enough 
    extract_mean_ROI_ts = pe.Node(interface = ExtractTS(plot_fig = True),name = 'extract_mean_ROI_ts')
